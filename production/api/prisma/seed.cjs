// Seed bản JS thuần — chạy bằng `node prisma/seed.cjs` trong image production
// (không cần ts-node/typescript, vốn là devDependency không có khi NODE_ENV=production).
...
async function main() {
  ...
  await prisma.user.upsert({
    where: { email },
    update: {},
    create: { email, passwordHash, name: process.env.ADMIN_NAME || 'Quản trị viên', role: 'SUPERADMIN' },
  });
  console.log(`OK Superadmin: ${email}`);
